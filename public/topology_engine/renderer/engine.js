...
class JSPowerMatrix {
        ...
        this.nodes.forEach(node => {
            ...
            if (node.type === "generator") {
                ...
                if (isPairA) {
                    ...
                    const starting = this.dg_pair_status === "pair_a_starting";
                    node.is_active = running || starting; node.kw_load = running ? 90 : 0;
                    node.status = running ? "RUNNING" : (starting ? "STARTING..." : "STANDBY");
                } else if (isPairB) {
                    const running = this.dg_pair_status === "pair_b_running";
                    const starting = this.dg_pair_status === "pair_b_starting";
                    node.is_active = running || starting; node.kw_load = running ? 90 : 0;
                    node.status = running ? "RUNNING" : (starting ? "STARTING..." : "STANDBY");
                }
                return;
    ...
    const getUpstreamTrace = (nodeId) => {
        ...
        const directFeederMap = {
            "node-dg-1": { paths: ["dg-cable-1"], devs: [] },
            "node-dg-2": { paths: ["dg-cable-2"], devs: [] },
            "node-dg-3": { paths: ["dg-cable-3"], devs: [] },
            "node-dg-4": { paths: ["dg-cable-4"], devs: [] },
            "node-dg-hq": { paths: ["dg-cable-hq"], devs: [] },
            ...
            entry.devs.forEach(dev => {
                if (!activeDevs.has(dev)) {
                    ...
                    if (dev === "node-main-main-db" && !engine.getGridActive() && _dgRunning) {
                        ...
                        activePaths.add("dg-bus");
                        // Only highlight the active pair's DG nodes and their cables
                        const pairStatus = typeof engine.getDgPairStatus === "function" ? engine.getDgPairStatus() : "";
                        const activeDGs = (pairStatus === "pair_b_running" || pairStatus === "pair_b_starting") ? [2, 4] : [1, 3];
                        activeDGs.forEach(j => {
                            activeDevs.add(`node-dg-${j}`);
                            activePaths.add(`dg-cable-${j}`);
                        });
                        
                        // Handle HQ generator
                        const hqNode = engine.getNode("node-dg-hq");
                        const hqActive = hqNode && hqNode.is_active;
                        if (hqActive) {
                            activeDevs.add("node-dg-hq");
                            activePaths.add("dg-cable-hq");
                        }
                    } else {
                        activeDevs.add(dev);
                        traverse(dev);
    ...
    const updateUIState = () => {
        ...
        const sidebarKillGridBtn = document.getElementById("sim-kill-grid");
        if (sidebarKillGridBtn) {
            sidebarKillGridBtn.textContent = gridActive ? "⚡ Kill ZESCO Grid" : "🔄 Restore ZESCO Grid";
        }
        ...
        svg.querySelectorAll("path[data-path-id]").forEach(path => {
            ...
            // Backup Gen Path — activate when specific DG pair is running or starting
            if (pathId === "dg-cable-1" || pathId === "dg-cable-3") {
                isPathActive = dgPairStatus === "pair_a_running" || dgPairStatus === "pair_a_starting" || dgPairStatus === "pair_b_starting";
            } else if (pathId === "dg-cable-2" || pathId === "dg-cable-4") {
                isPathActive = dgPairStatus === "pair_b_running" || dgPairStatus === "pair_b_starting";
            } else if (pathId === "dg-cable-hq") {
                const hqNode = engine.getNode("node-dg-hq");
                isPathActive = hqNode && hqNode.is_active;
            } else if (pathId.startsWith("dg-")) {
