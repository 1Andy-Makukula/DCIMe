...
export function MetricTile({
  label, value, unit, status = null, decimals = 1, footnote, children, onClick
}: MetricTileProps) {
  const shown =
  ...
  return onClick ? (
    <button
      onClick={onClick}
      className="rounded-2xl border border-neutral-200 bg-white p-4 text-left shadow-sm transition-colors hover:border-neutral-300 focus-visible:outline-2 focus-visible:outline-brand-500"
    >
      {body}
    </button>
  ) : (
    <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">{body}</div>
  );
}
