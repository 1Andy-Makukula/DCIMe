...
export const generateReportTexts = ({
  ...
  // ── Report body ──
  // Every site renders the same full report. A second, abbreviated layout used
  // to exist for any site other than the default one, and it dropped rectifier
  // 2, UPS 2, power room 2, the second enterprise room, the PAC unit readings
  // and the grid metrics entirely — which is why history for those sites came
  // back cut short. There is one layout now; a site missing a unit prints NA
  // for it rather than losing the whole section.
  ...
  // The default site is identified by its code; every other site carries its
  // own stored name, which siteLabel passes through unchanged.
  const headerLabel = siteCode === DEFAULT_SITE_CODE
    ? siteLabel(siteCode)
...
Humidity: ${humidityMain}%`;
  ...
  const whatsappPayload = `${prefixWA}${commonBody}`;
  ...
  const internalPayload = `${finalInternalPayload}
