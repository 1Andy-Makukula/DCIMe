...
export function ManualEntryForm({ kind, busy, onStage }: ManualEntryFormProps) {
  ...
  const field = (c: string, required: boolean) => (
    <div key={c}>
      <label className="mb-1 block font-mono text-[9px] uppercase tracking-wider text-neutral-400">
      ...
      <input
        ...
        placeholder={HINTS[c] ?? ""}
        className="w-full rounded-xl border-2 border-neutral-100 bg-white px-3 py-2 text-[12px] font-semibold text-neutral-900 outline-none transition-colors focus:border-brand-400"
      />
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-4">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          <p className="text-[13px] font-black text-neutral-900">Type it in</p>
      ...
      <button
        ...
        className="mt-2.5 text-[10px] font-black uppercase tracking-wider text-neutral-400 transition-colors hover:text-neutral-700"
      >
      ...
      {queued.length > 0 && (
        <div className="mt-3 space-y-1 border-t border-neutral-100 pt-3">
          {queued.map((r, i) => (
            <div key={i} className="flex items-center gap-2 rounded-lg bg-neutral-50 px-3 py-2">
              <span className="font-mono text-[10px] text-neutral-400">{i + 1}</span>
              <span className="min-w-0 flex-1 truncate font-mono text-[11px] font-semibold text-neutral-700">
                {cols.required.map(c => r[c]).filter(Boolean).join("  ·  ")}
              </span>
              <button
                onClick={() => setQueued(q => q.filter((_, j) => j !== i))}
                aria-label={`Remove row ${i + 1}`}
                className="shrink-0 rounded text-neutral-300 transition-colors hover:text-danger-500"
              >
                <Trash2 size={13} />
              </button>
            </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2 border-t border-neutral-100 pt-3">
        <button
          onClick={addRow}
          disabled={!complete}
          className="flex items-center gap-1.5 rounded-xl border-2 border-neutral-200 px-4 py-2.5 text-[11px] font-black uppercase tracking-wider text-neutral-700 transition-colors hover:bg-neutral-50 disabled:border-neutral-100 disabled:text-neutral-300"
        >
        ...
        <button
          ...
          className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-neutral-900 px-5 py-2.5 text-[11px] font-black uppercase tracking-wider text-white transition-colors hover:bg-neutral-700 disabled:bg-neutral-200 disabled:text-neutral-400 sm:flex-none"
        >
