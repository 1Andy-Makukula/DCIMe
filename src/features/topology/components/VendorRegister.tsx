...
function VendorRow({ v, onSave, onFlag, onSetActive, onDelete }: {
  ...
  return (
    <div className={`rounded-2xl border bg-white p-4 ${
      ...
      : !v.is_active   ? "border-neutral-200 opacity-70"
      : "border-neutral-200"
    }`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button
          ...
          className="shrink-0 rounded-lg border border-neutral-200 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-neutral-500 hover:bg-neutral-50"
        >
          {editing ? "Cancel" : "Edit"}
        </button>
      </div>
      ...
      <div className="mt-3 grid grid-cols-4 gap-2">
        {([
          ...
          <div key={label} className="rounded-xl bg-neutral-50 p-2 text-center">
            <p className={`text-[16px] font-black tabular-nums leading-none ${cls || "text-neutral-900"}`}>{n}</p>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5 border-t border-neutral-100 pt-3">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg border border-neutral-200 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-neutral-500 transition-colors hover:bg-neutral-50"
        >
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg border border-neutral-200 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-danger-500 transition-colors hover:bg-danger-50 disabled:cursor-not-allowed disabled:border-neutral-100 disabled:text-neutral-300 disabled:hover:bg-transparent"
        >
      ...
      {flagging && (
        <div className="mt-2 flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-xl border border-warn-200 bg-warn-50/50 px-3 py-2 text-[12px] text-neutral-900 focus:border-warn-400 focus:outline-none"
          />
          <button
            onClick={() => { onFlag(flagWhy); setFlagging(false); setFlagWhy(""); }}
            disabled={!flagWhy.trim()}
            className="rounded-xl bg-warn-500 px-3 text-[11px] font-black uppercase tracking-wider text-white disabled:bg-neutral-200 disabled:text-neutral-400"
          >
            Flag
          </button>
          <button onClick={() => { setFlagging(false); setFlagWhy(""); }} aria-label="Cancel"
            className="rounded-xl px-2 text-neutral-400 hover:bg-neutral-100"><X size={15} /></button>
        </div>
      )}
        ...
        <div className="mt-3 flex flex-col gap-2 border-t border-neutral-100 pt-3">
          <div className="grid grid-cols-2 gap-2">
            <input value={contact} onChange={e => setContact(e.target.value)}
              ...
              className="rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-2 text-[12px] focus:border-neutral-400 focus:outline-none" />
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <input value={sla} onChange={e => setSla(e.target.value)}
              ...
              className="rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-2 text-[12px] tabular-nums focus:border-neutral-400 focus:outline-none" />
          </div>
  ...
  if (error) {
    return (
      <div className="flex min-h-[16rem] flex-col items-center justify-center gap-3 p-6 text-center">
        ...
        <p className="text-[13px] font-bold text-neutral-800">Could not load the vendor register</p>
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <button onClick={() => setAdding(a => !a)}
          className="flex items-center gap-2 rounded-xl border border-neutral-300 px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-neutral-700 hover:bg-neutral-50">
          <Plus size={14} /> Add vendor
        </button>
      </div>

      {adding && (
        <div className="flex gap-2 rounded-2xl border border-neutral-200 bg-neutral-50 p-3">
          <input value={name} onChange={e => setName(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter") add(); }}
            placeholder="Company name"
            className="min-w-0 flex-1 rounded-xl border border-neutral-200 bg-white px-3 py-2.5 text-[12px] focus:border-neutral-400 focus:outline-none" />
          <button onClick={add}
            className="rounded-xl bg-neutral-900 px-4 text-[12px] font-bold text-white">Add</button>
          <button onClick={() => setAdding(false)} aria-label="Cancel"
            className="rounded-xl px-2 text-neutral-400 hover:bg-neutral-200"><X size={15} /></button>
        </div>
      )}

      {vendors.length === 0 ? (
        <div className="flex min-h-[14rem] flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-neutral-300 text-center">
          <Inbox size={26} className="text-neutral-300" />
          <p className="text-[13px] font-bold text-neutral-700">No vendors yet</p>
